    ...
    const load = async () => {
      ...
      try {
        const [svcs, profs] = await Promise.all([
          sb.select("servicos", "?select=*&order=name"),
          sb.select("profissionais", "?select=*,profiles(full_name)&order=rating.desc&limit=4"),
        ]);
...
function OrdersScreen({ goTo, setCtx, user }) {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const data = await sb.select("pedidos", `?select=*,servicos(name,icon),profiles!professional_id(full_name)&client_id=eq.${user?.id}&order=created_at.desc`);
...
function BookingScreen({ goTo, ctx, user }) {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const res = await sb.insert("pedidos", orderData);
...
function ChatScreen({ goTo, ctx, user }) {
  ...
  useEffect(() => {
    ...
    if (ctx?.orderId && !user?.demo) {
      sb.select("mensagens", `?order_id=eq.${ctx.orderId}&order=created_at`).then(data => {
  ...
  const send = async () => {
    ...
    // Salva no Supabase
    if (ctx?.orderId && !user?.demo) {
      sb.insert("mensagens", { order_id: ctx.orderId, sender_id: user?.id, text: input });
    }
...
function AdminScreen() {
  ...
  useEffect(() => {
    sb.select("pedidos","?select=price,status").then(data => {
      ...
      <div style={{ padding:20 }}>
        ...
        {tab==="db" && (
          <>
            ...
            {["perfis","categorias","serviços","profissionais","endereços","pedidos","pagamentos","avaliações","mensagens","notificações","atualizações_de_localização"].map(t=>(
